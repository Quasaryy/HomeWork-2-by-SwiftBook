...
var todayOfDay = 2
var todayOfMonth = 8
var todayOfYear = 2023


let yearsPassed = todayOfYear - yearOfBirth // Прошло лет
let monthPassed = yearsPassed * 12 - 4 // Прошло месяцев
let daysPassed = monthPassed * 30 - 30 // Прошло дней
let secondsPassed = daysPassed * 86400 // Прошло секунд


// Прошло времени с момента рождения
...
let percentOfpayout = 0.10
let deposit: Double = 500_000
let yersOfDeposit: Double = 5

let totalPayout = (percentOfpayout * deposit * yersOfDeposit)
let totalPayoutInPercent = (totalPayout / deposit) * 100

print("Выплата по депозиту суммой \(deposit) (ставка \(percentOfpayout * 100)%) через \(yersOfDeposit) лет будет \(totalPayoutInPercent)%")
